...
class EvalArguments:
    ...
    data_path: str = field(default="spin_data/spin_iter0.json")
...
class MetaEvalDataset(Dataset):
    def __init__(self, args: EvalArguments, tokenizer):
        ...
        print("Filtered", self.__len__(), "for generation")
...
TASK_MAP = {
    "foward_pred": EvalForwardPredDataset,
    "reagent_pred": EvalReagentPredDataset,
    "retrosynthesis": EvalRetrosynDataset,
    "property_pred": EvalPropertyPredDataset,
...
def start_eval(args: EvalArguments):
    ...
    for task in tasks:
        ...
        args.data_path = os.path.join(parent, dataset_files[position])
        data = TASK_MAP[task](tokenizer=tokenizer, args=args)
        datasets.append(data)
        
    datasets = ConcatDataset(datasets)
    loader = DataLoader(datasets, batch_size=args.batch_size, shuffle=False, collate_fn=GraphEvalCollator(tokenizer), drop_last=False)
    output = []
    for idx, batch in enumerate(tqdm(loader, desc="inference")):
        ...
        for idx, (result, input_id, prompt, gt) in enumerate(zip(output_ids, input_ids, batch["prompt"], batch["gt"])):
            this_output = {
                "real": [{"role": "human", "value": batch["instruction"]}, {"role": "gpt", "value": gt}], 
                "generated": [{"role": "human", "value": batch["instruction"]}, {"role": "gpt", "value": tokenizer.decode(result[input_id.shape[0]:], skip_special_tokens=True)}]
            }
