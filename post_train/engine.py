...
from accelerate.utils import is_deepspeed_available, is_peft_available, is_wandb_available
...
class SPINTrainer(Trainer):
    ...
    def _get_batch_logps(
        ...
        if logits.shape[:-1] != labels.shape:
            raise ValueError(f"Logits (batch and sequence length dim) and labels must have the same shape. But found {logits.shape[:-1]} and {labels.shape}")
    ...
    def concatenated_forward(
        ...
        batch = {"real_" + k: v for k, v in real_batch.items() if "graphs" not in k}
        batch.update({"generated_" + k: v for k, v in generated_batch.items() if "graphs" not in k})
        ...
        assert not self.is_encoder_decoder, "Only supports GPT like model"
        output = model(
            input_ids=concatenated_batch["concatenated_input_ids"],
            attention_mask=concatenated_batch["concatenated_attention_mask"],
            graphs=concatenated_graph,
            labels = concatenated_batch["concatenated_labels"]
        )
        all_logits = output.logits.float()
